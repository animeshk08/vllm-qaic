...
def check_outputs_equal(outputs_0, outputs_1, name_0, name_1):
    ...
    for i, (o0, o1) in enumerate(zip(outputs_0, outputs_1)):
        ids0, text0 = o0
        ids1, text1 = o1
        fail_msg = f"Test{i}:\n{name_0}:\t{text0!r}\n{name_1}:\t{text1!r}"
...
def seq_len(request, pytestconfig):
    return _resolve_qaic_test_config_value(request, pytestconfig, "seq_len", default=128)
...
def decode_bsz(request, pytestconfig):
    return _resolve_qaic_test_config_value(
        request, pytestconfig, "decode_bsz", default=pytestconfig.getoption("decode_bsz")
    )
...
def kv_dtype(request, pytestconfig):
    return _resolve_qaic_test_config_value(request, pytestconfig, "kv_dtype", default="auto")
...
def num_prompt(request, pytestconfig):
    return _resolve_qaic_test_config_value(request, pytestconfig, "num_prompt", default=20)
...
def qaic_model(
    ...
    seq_len = _resolve_qaic_test_config_value(request, pytestconfig, "seq_len", default=128)
    ...
    kv_dtype = _resolve_qaic_test_config_value(request, pytestconfig, "kv_dtype", default="auto")
...
def pytest_collection_modifyitems(session, config, items):
    ...
    for item in items:
        ...
        if required > pool_size:
            item.add_marker(
                pytest.mark.skip(
                    reason=f"not enough devices in pool: needs {required}, pool has {pool_size}"
                )
            )

        if item.get_closest_marker(
            "qaic_aot_mode"
        ) is not None and not current_platform.is_aot_inference():
            item.add_marker(
                pytest.mark.skip(
                    reason="AOT mode is not installed"
                )
            )

        if item.get_closest_marker(
            "qaic_disagg_installed"
        ) is not None and importlib.util.find_spec("qaic_disagg") is None:
            item.add_marker(
                pytest.mark.skip(
                    reason="qaic_disagg package is not installed"
                )
            )

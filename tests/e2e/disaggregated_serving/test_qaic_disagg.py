...
class TestDisaggregatedServingBasic:
    ...
    def test_data_consistency(self, disagg_server):
        ...
        def get_responses_map(input_prompts):
            ...
            assert len(response_json["choices"]) == len(input_prompts), (
                f"Expected {len(input_prompts)} choices, got {len(response_json['choices'])}"
            )
            return {
                tuple(p) if isinstance(p, list) else p: response_json["choices"][i]["text"]
    ...
    def test_parallel_requests(
        ...
        # Using `itertools.cycle` ensures we don't run out of prompts if `num_requests` is large.
        print(
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in tqdm(
                ...
                try:
                    ...
                    if num_requests == 50:
                        pytest.fail(
                            f"Failed Processing the Requests due to timeout: {timeout}: {e}"
                        )
    ...
    def test_generation_length_one(self, disagg_server, stream):
        ...
        num_prompts = 20
        prompts = random.sample(
            get_prompts("/v1/completions", ctx_len=test_config["ctx_len"], max_tokens=1),
            num_prompts,
        )
                ...
                try:
                    ...
                    if stream:
                        ...
                        assert len(parsed_packets) == 2, (
                            f"Expected exactly 2 packets (token + DONE), got {len(parsed_packets)}"
                        )
                        ...
                        assert choice["finish_reason"] == "length", (
                            f"Unexpected finish_reason {choice['finish_reason']} in token packet"
                        )
                    else:
                        response_json = response.json()
                        ...
                        assert response_json["usage"]["completion_tokens"] == 1, (
                            f"Expected 1 completion token but got {response_json['usage']['completion_tokens']} for prompt: {prompt}"
                        )
                        assert (

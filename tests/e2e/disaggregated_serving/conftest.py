...
def _disagg_test_config(request, pytestconfig, host, port, device_pool_ids):
    ...
    ids = _device_pool.acquire(device_pool_ids, num_prefill_devices + num_decode_devices)
...
def disagg_server(request, _disagg_test_config):
    ...
    logging.info(f"Starting disaggregated server (model: {model_name}, port: {port})...")
    ...
    # --- Teardown ---
    logging.info(f"Shutting down disaggregated server (model: {model_name}, port: {port})...")
    process.send_signal(signal.SIGINT)
    try:
        process.wait(timeout=300)
    except subprocess.TimeoutExpired:
        logging.warning(
            f"Server {model_name} on port {port} did not terminate gracefully, forcing kill."
        )
        process.kill()
    ...
    for f in current_dir.glob("qaic_*.log"):
        try:
            ...
            logging.warning(f"Error moving log file {f.name} to {disagg_log_path}: {e}")

...
def _print_kpi_comparison(
    ...
    def tpot(dec_times, toks):
        vals = [d / (t - 1) for d, t in zip(dec_times, toks) if t > 1]
    ...
    print(f"  {'-' * 56}")
    print(
        f"  {'Avg TTFT (ms)':<28} {avg(s_ttft) * 1000:>13.1f}  {avg(a_ttft) * 1000:>13.1f}"
    )
    print(
        f"  {'Avg Decode Time (ms)':<28} {avg(s_dec) * 1000:>13.1f}  {avg(a_dec) * 1000:>13.1f}"
    )
    print(
        f"  {'Avg E2E Latency (ms)':<28} {avg(s_e2e) * 1000:>13.1f}  {avg(a_e2e) * 1000:>13.1f}"
    )
    print(
        f"  {'Avg TPOT (ms/tok)':<28} {tpot(s_dec, s_toks):>13.2f}  {tpot(a_dec, a_toks):>13.2f}"
    )
    print(
        f"  {'Throughput (tok/s)':<28} {throughput(s_e2e, s_toks):>13.1f}  {throughput(a_e2e, a_toks):>13.1f}"
    )
    print(
        f"  {'Decode Throughput (tok/s)':<28} {decode_throughput(s_dec, s_toks):>13.1f}  {decode_throughput(a_dec, a_toks):>13.1f}"
    )
    print(f"  {'Avg Output Tokens':<28} {avg(s_toks):>13.1f}  {avg(a_toks):>13.1f}")
    print(f"  {'-' * 56}")
    print(
        f"  {'Total Exec Time (s)':<28} {sync_total_time:>13.2f}  {async_total_time:>13.2f}"
    )
    print(f"{'=' * 62}\n")
...
@pytest.mark.skip(reason="async scheduling is currently broken; re-enable once fixed")
...
def test_output_consistency_database_chunked_prefill(
    ...
    with (
        ...
        _make_runner_chunked(make_runner, True, device_groups[1], seq_len) as runner_async,
    ):
        _run_consistency_check(prompts, runner, runner_async, sampling_params)

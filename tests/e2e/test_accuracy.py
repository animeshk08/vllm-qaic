...
def get_groundtruth(processed_dataset_file) -> pd.Series:
    data: pd.DataFrame = pd.read_pickle(processed_dataset_file)
    ground_truths: pd.Series = data["output"]
    return ground_truths


def load_mlperf_prompts(dataset_path: str, num_seqs: int) -> list[str]:
    data: pd.DataFrame = pd.read_pickle(dataset_path)
    return data["input"].tolist()[:num_seqs]
...
def compute_mlperf_rouge(
    ...
    targets = get_groundtruth(dataset_path)[: len(vllm_preds)]

    target_required = []
    preds_token_ids = []

    gen_tok_len = 0
    for pred, target in zip(vllm_preds, targets):
...
class TestAccuracy:
    @pytest.mark.qaic_test_config(
        model_name="TinyLlama/TinyLlama-1.1B-Chat-v1.0", ctx_len=256, dtype="mxfp6", kv_dtype="mxint8"
    )
    def test_accuracy(self, qaic_model, sampling_params, model_name, decode_bsz, device_pool_ids):
        llm = qaic_model.get_llm()
        mlperf_dataset = os.path.join(
            os.path.dirname(__file__),
            "dataset/open_orca_gpt4_tokenized_llama3.sampled_1024new_truncated_prompts_to_128.pkl",
        )
        inputs = load_mlperf_prompts(mlperf_dataset, num_seqs=decode_bsz)

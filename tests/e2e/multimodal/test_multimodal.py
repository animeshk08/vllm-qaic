...
class _DualQpcTestBase:
    ...
    def test_dual_qpc_single_image(
        self, mm_input, model_name, device_groups, make_runner, decode_bsz, ctx_len,
        seq_len, dtype, kv_dtype,
    ):
        ...
        with make_runner(
            async_scheduling=False, dg=device_groups[0], max_num_seqs=1,
            runner="pooling", quantization=None, kv_cache_dtype="auto",
            override_qaic_config=override_cfg, trust_remote_code=is_internvl(model_name),
            enable_mm_embeds=True, limit_mm_per_prompt={"image": 1}, **mm_kwargs,
        ) as qllm_embed, make_runner(
            async_scheduling=False, dg=device_groups[1], max_num_seqs=decode_bsz,
            override_qaic_config=override_cfg, trust_remote_code=is_internvl(model_name),
            enable_mm_embeds=True, limit_mm_per_prompt={"image": 1}, **mm_kwargs,
        ) as qllm_gen:
            inputs = [
        ...
        qeff_model, processor = create_qeff_model_and_processor(
            model_name, device_groups[1], seq_len, ctx_len, dtype, kv_dtype,
            kv_offload=True, override_cfg=override_cfg, tokenizer_obj=tokenizer,
        )
        ...
        for img, p in updated_input:
            ...
            if is_qwenvl(model_name):
                inputs_proc = qeff_model.model.prepare_inputs_for_generation(
                    inputs=inputs_proc, prefill_seq_len=seq_len, batch_size=decode_bsz,
                )
            output = qeff_model.generate(
                inputs=inputs_proc, device_ids=device_groups[1],
        ...
        for o1, o2 in zip(qllm_output, qeff_output):
            assert len(o1) > 1
            o1 = o1.strip()
    ...
    def test_dual_qpc_single_image_cb(
        self, mm_input, model_name, device_groups, make_runner, decode_bsz,
    ):
        ...
        with make_runner(
            ...
            async_scheduling=False, dg=device_groups[1], max_num_seqs=1,
            override_qaic_config=override_cfg, trust_remote_code=is_internvl(model_name),
            enable_mm_embeds=True, limit_mm_per_prompt={"image": 1}, **mm_kwargs,
        ) as qllm_gen:
        ...
        for o1, o2 in zip(outputs, outputs_cb):
    ...
    def test_dual_qpc_text_only(
        self, text_input, model_name, device_groups, make_runner, decode_bsz,
    ):
        ...
        for op, ip in zip(outputs, repeated_inputs):
    ...
    def test_dual_qpc_mixed_modality(
        self, mm_input, text_input, model_name, device_groups, make_runner, decode_bsz,
    ):
...
def _multi_image_test(
    ...
    built_input = build_model_input(model_name, multi_image_input, tokenizer, num_images=2)
    inputs = [
    ...
    with make_runner(
        ...
        runner="pooling", quantization=None, kv_cache_dtype="auto",
        override_qaic_config=override_cfg, trust_remote_code=is_internvl(model_name),
        enable_mm_embeds=True, limit_mm_per_prompt={"image": 2}, **mm_kwargs,
    ) as qllm_embed, make_runner(
        async_scheduling=False, dg=device_groups[1], max_num_seqs=decode_bsz,
        override_qaic_config=override_cfg, trust_remote_code=is_internvl(model_name),
        enable_mm_embeds=True, limit_mm_per_prompt={"image": 2}, **mm_kwargs,
...
def _qwenvl_multi_resolution_test(
    ...
    updated_input = build_model_input(
        ...
        [
            ...
            for (img, question), (h, w) in zip(mm_input, random_sizes)
        ],
        tokenizer,
    )
    inputs = [
...
@pytest.mark.qaic_test_config(
    model_name="Qwen/Qwen3-VL-2B-Instruct", ctx_len=4096, dtype="mxfp6",
    kv_dtype="mxint8", num_device_groups=2, device_group_size=1,
)
...
@pytest.mark.qaic_test_config(
    model_name="Qwen/Qwen2.5-VL-2B-Instruct", ctx_len=4096, dtype="mxfp6",
    kv_dtype="mxint8", num_device_groups=2, device_group_size=1,
)
...
@pytest.mark.qaic_test_config(
    model_name="OpenGVLab/InternVL2_5-1B", ctx_len=4096, dtype="mxfp6",
    kv_dtype="mxint8", num_device_groups=2, device_group_size=1,
)
...
@pytest.mark.qaic_test_config(
    model_name="llava-hf/llava-interleave-qwen-0.5b-hf", ctx_len=4096, dtype="mxfp6",
    kv_dtype="mxint8", num_device_groups=2, device_group_size=1,
)
...
@pytest.mark.qaic_test_config(
    model_name="google/gemma-3-4b-it", ctx_len=4096, dtype="mxfp6",
    kv_dtype="mxint8", num_device_groups=2, device_group_size=1,
)
...
@pytest.mark.qaic_test_config(
    model_name="ibm-granite/granite-vision-3.2-2b", ctx_len=6144, dtype="mxfp6",
    kv_dtype="mxint8", num_device_groups=2, device_group_size=1,
)
...
@pytest.mark.qaic_test_config(
    model_name="openai/whisper-tiny.en", ctx_len=448, dtype="mxfp6",
    kv_dtype="mxint8", num_device_groups=1, device_group_size=1,
)
def test_whisper(audio_data, model_name, device_group, make_runner, decode_bsz, ctx_len):
    ...
    with make_runner(
        async_scheduling=False, dg=device_group, max_num_seqs=decode_bsz,
